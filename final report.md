...
# Ⅳ 优化方案
## 1 多聊天室
### 目前多聊天室的实现方式是：用户登录时输入聊天室号码——服务器数据库记录此次登录进入的聊天室——客户端向服务器发送消息——服务器仅向进入输入相同聊天室号的客户端推送消息。当前实现方式存在的弊端在于：用户每次登录只能进入一个聊天室，如需切换则需要重新登陆；用户进入聊天室没有限制，可随意进入他人聊天室，安全性较低。综合这两个缺点，当前的设计需要组织内部预先分配聊天室号码。
### 可能的优化方案：
